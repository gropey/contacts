...
sub make_sql {
 ...
 DATE_FORMAT(c.callback,'%y%m%d') AS sdate,IF(c.reminder = '1' OR c.callback = CURDATE(),'1','0') AS reminder,clients.cid,=srcCode= 
...
# IF(dnc.cid AND clients.cid IS NULL,concat(left(c.phone,length(c.phone)-3),'DNC'),c.phone) AS phone,c.contacts,=comment= AS comment,
 push @where, "c.usrid = $prm->{id}";
   ...
   elsif ($prm->{filteron} =~ /^[a-zA-Z]$/ && $prm->{cat} eq 'clicks' && $prm->{cat} !~ /contacts/) {
      push @where, "$phash{ 'organization' } REGEXP ?";
   }
   elsif (!$prm->{filteron} && $prm->{cat}) {
 ...
 if ($prm->{cat} eq "notes") {
     ...
     #$sql =~ s/=comment=/concat(clicks.clk_date,'  ',clicks.url)/;
